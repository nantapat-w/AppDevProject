...
const Chat = () => {
  ...
  useEffect(() => {
    ...
    if (activeChat && activeChat._id !== 'new_temp_chat' && myId) {
      const fetchMessages = async () => {
        try {
          const res = await axios.get(`/chats/${activeChat._id}`, { withCredentials: true });
  ...
  useEffect(() => {
    ...
    const fetchTrackedTrades = async () => {
      try {
        const res = await axios.get('/trades/outbox', { withCredentials: true });
        const res2 = await axios.get('/trades/inbox', { withCredentials: true });
    ...
    try {
      ...
      if (res.data.success) {
        ...
        if (activeChat._id === 'new_temp_chat') {
            const refreshRes = await axios.get('/chats', { withCredentials: true }); 
  ...
  const updateShippingStatus = async () => {
    try {
        const res = await axios.put('/trades/shipping', {
  ...
  const completeTradeTransaction = async (tradeId) => {
    try {
        const res = await axios.put(`/trades/complete/${tradeId}`, {}, { withCredentials: true });
        if (res.data.success) {
